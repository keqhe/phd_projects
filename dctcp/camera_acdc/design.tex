...
bits from packets in order to prevent the VM's TCP stack from decreasing rates too aggressively
...
Since the VM's TCP stack may not support DCTCP, the~\acdc{} receiver module monitors
the total and ECN-marked bytes received for a flow. Receivers piggy-back the
reported totals on ACKs to the sender by adding an additional 8 bytes as a TCP Option. This is called 
...
If adding a PACK creates a packet larger than the MTU size, the NIC offload feature (\ie{}, TSO)
...
size will be violated. The FACK is sent in addition to the real TCP ACK (RACK).
...
The VM's TCP flow then uses $min(\cwnd{},\rwnd{})$ to limit how many packets it can send.

Ensuring a VM's TCP flows adhere to the~\rwnd{} value is relatively simple. The vSwitch calculates a new congestion 
window every time an ACK is received. This value provides a bound on the number of bytes the VM's TCP flow
...
While this scheme seems simple, it provides a surprising amount of flexibility. For example, TCP enables
...
Additionally, the sender module can generate duplicate ACKs to send to a VM's TCP flow. This is useful when 
...
cannot guarantee the VM flow's~\cwnd{} will allow the flow to increase its sending rate, whereas
